...
public class KeyboardAndDisplaySimulator extends AbstractMarsToolAndApplication {
	...
	// or random access (row, column).  Supports new random access feature. 17-July-2014
	...
	*/
	...
	// each time to reflect possible change in memory configuration. 6-Aug-09
	protected void initializePreGUI() {
		...
		keyboardPanelTitle = "KEYBOARD: Characters typed here are stored to Receiver Data "
	...
	*/
	protected void processMIPSUpdate(Observable memory, AccessNotice accessNotice) {
		...
		if (isReadyBitSet(TRANSMITTER_CONTROL) && notice.getAddress() == TRANSMITTER_DATA
		...
		if (characterToDisplay == CLEAR_SCREEN)
			...
			if (displayRandomAccessMode) {
				try {
					...
					display.replaceRange("" + characterToDisplay, display.getCaretPosition(),
							display.getCaretPosition() + 1);
				}
			} else
	...
	protected JComponent getHelpComponent() {
		final String helpContent = "Keyboard And Display MMIO Simulator\n\n" +
				...
				+ Binary.intToHexString(RECEIVER_CONTROL) + ").  The Ready " +
				...
				+ Binary.intToHexString(TRANSMITTER_DATA) + ") using a 'sw' instruction.  This " +
		...
		help.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						// do it the hard way.  16-July-2014
						final JDialog d;
						...
						d.setVisible(true);
						// This alternative technique is simpler than the above but requires java 1.6!  16-July-2014
	...
	private synchronized void updateMMIOControlAndData(int controlAddr, int controlValue, int dataAddr, int dataValue,
			boolean controlOnly) {
	...
	private class KeyboardKeyListener implements KeyListener {
		public void keyTyped(KeyEvent e) {
			...
			updateMMIOControlAndData(RECEIVER_CONTROL, updatedReceiverControl, RECEIVER_DATA,
	...
	private class FontChanger implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			// do nothing, font isnt even used suprisingly lol, someone could try implementing but eh
		}
	}
}