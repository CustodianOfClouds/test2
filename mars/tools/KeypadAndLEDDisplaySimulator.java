...
public class KeypadAndLEDDisplaySimulator extends AbstractMarsToolAndApplication {
	...
	*/
	...
	private boolean okayToWriteToMemory() {
		return !this.isBeingUsedAsAMarsTool || (this.connectButton != null && this.connectButton.isConnected());
	}
	...
	private static class ClassicLEDDisplayPanel extends LEDDisplayPanel {
		...
		private BufferedImage image = new BufferedImage(N_COLUMNS, N_ROWS, BufferedImage.TYPE_INT_RGB);
				...
				public void keyPressed(KeyEvent e) {
					switch (e.getKeyCode()) {
						case KeyEvent.VK_LEFT:
							changeKeyState(keyState | KEY_L);
							break;
						case KeyEvent.VK_RIGHT:
							changeKeyState(keyState | KEY_R);
							break;
						case KeyEvent.VK_UP:
							changeKeyState(keyState | KEY_U);
							break;
						case KeyEvent.VK_DOWN:
							changeKeyState(keyState | KEY_D);
							break;
						case KeyEvent.VK_B:
							changeKeyState(keyState | KEY_B);
							break;
						case KeyEvent.VK_Z:
							changeKeyState(keyState | KEY_Z);
							break;
						case KeyEvent.VK_X:
							changeKeyState(keyState | KEY_X);
							break;
						case KeyEvent.VK_C:
							changeKeyState(keyState | KEY_C);
							break;
						default:
							break;
					}
	...
	private static class EnhancedLEDDisplayPanel extends LEDDisplayPanel {
		...
		// Intermediate images
		private WritableRaster fullTmLayerLo = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, TM_PIXEL_W, TM_PIXEL_H,
				1, null);
		private WritableRaster fullTmLayerHi = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, TM_PIXEL_W, TM_PIXEL_H,
				1, null);

		// Big enough to accommodate the largest sprites partially offscreen on all 4 sides
		private WritableRaster fullSpriteLayer = Raster.createBandedRaster(DataBuffer.TYPE_BYTE,
				N_COLUMNS + (2 * SPRITE_LARGE_W), N_ROWS + (2 * SPRITE_LARGE_H), 1, null);

		// Compositing layers
		private WritableRaster fbLayer = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, N_COLUMNS, N_ROWS, 1, null);
		private WritableRaster tmLayerLo = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, N_COLUMNS, N_ROWS, 1, null);
		private WritableRaster tmLayerHi = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, N_COLUMNS, N_ROWS, 1, null);
		private WritableRaster spriteLayer = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, N_COLUMNS, N_ROWS, 1,
				null);
		private WritableRaster finalRaster = Raster.createBandedRaster(DataBuffer.TYPE_BYTE, N_COLUMNS, N_ROWS, 1,
				null);
		private BufferedImage finalImage = new BufferedImage(N_COLUMNS, N_ROWS, BufferedImage.TYPE_INT_RGB);
		...
		public EnhancedLEDDisplayPanel(KeypadAndLEDDisplaySimulator sim) {
			...
			this.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					// TODO: possibly also do text input
				}
			...
			this.addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					if (SwingUtilities.isLeftMouseButton(e))
						mouseButtons |= MOUSE_LBUTTON;
					else if (SwingUtilities.isRightMouseButton(e))
						mouseButtons |= MOUSE_RBUTTON;
					else if (SwingUtilities.isMiddleMouseButton(e))
						mouseButtons |= MOUSE_MBUTTON;
				}
				...
				case 0:
					if (addr >= DISPLAY_PALETTE_RAM) {
						...
						switch (addr) {
							...
							case DISPLAY_FRAME_COUNTER:
								this.updateFrameCounterRegister();
								break;

							case DISPLAY_FB_CLEAR:
								this.clearFb();
								break;
							case DISPLAY_FB_IN_FRONT:
								this.fbInFront = value != 0;
								break;
							case DISPLAY_FB_PAL_OFFS:
								this.setFbPalOffs(value);
								break;
							case DISPLAY_FB_SCX:
								this.setFbScx(value);
								break;
							...
							case DISPLAY_KEY_HELD:
								this.updateKeyHeld(value);
								break;
							case DISPLAY_KEY_PRESSED:
								this.updateKeyPressed(value);
								break;
							case DISPLAY_KEY_RELEASED:
								this.updateKeyReleased(value);
								break;
							case DISPLAY_MOUSE_VISIBLE:
								this.showCursor(value != 0);
								break;
							default:
								break;
						}
		...
		private void initializePaletteRam() {
			...
			synchronized (Globals.memoryAndRegistersLock) {
				for (int i = 0; i < 256; i++) {
					int color = ((((int) paletteRam[0][i]) & 0xFF) << 16) |
