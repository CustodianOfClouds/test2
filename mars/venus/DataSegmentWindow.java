...
public class DataSegmentWindow extends JPanel implements Observer {
	...
	// MEMORY_CHUNK_SIZE means scroll a full table's worth.  Scrolls through memory faster.  26-Jan-09
	...
	// when the corresponding combo box item is selected.  22-Nov-2006
	...
	*/
	void highlightCellForAddress(int address) {
		...
		this.addressRowFirstAddress = Binary
				.stringToInt(dataTable.getValueAt(this.addressRow, ADDRESS_COLUMN).toString());
	...
	private int getBaseAddressIndexForAddress(int address) {
		...
		else if (Memory.inTextSegment(address)) // 8-July-2013
	...
	public void clearHighlighting() {
		...
		// initially false since column>=0.  23 jan 2009
		addressColumn = -1;
	}
	...
	public void updateModelForMemoryRange(int firstAddr) {
		...
		for (int row = 0; row < NUMBER_OF_ROWS; row++) {
			((DataTableModel) dataModel).setDisplayAndModelValueAt(
					NumberDisplayBaseChooser.formatUnsignedInteger(address, addressBase), row, ADDRESS_COLUMN);
			for (int column = 1; column < NUMBER_OF_COLUMNS; column++) {
				try {
					((DataTableModel) dataModel).setDisplayAndModelValueAt(
					...
					// Now it becomes the "else" part, executed when not in text segment.  8-July-2014.
					else
						((DataTableModel) dataModel).setDisplayAndModelValueAt(
	...
	*/
	...
	private void addButtonActionListenersAndInitialize() {
		...
		globButton.addActionListener(
				new ActionListener() {
					public void actionPerformed(ActionEvent ae) {
						...
						firstAddress = Math.max(Memory.dataSegmentBaseAddress,
		...
		//       specialized inner classes at the bottom of this listing.  20 July 2008
	...
	*/
	...
	public void update(Observable observable, Object obj) {
		if (observable == mars.simulator.Simulator.getInstance()) {
			...
			// Suspended work in progress. Intended to disable combobox item for text segment. 9-July-2013.
	...
	// "fireContentsChanged()" method worked. 25-Jan-2009
