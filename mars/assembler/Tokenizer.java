...
public class Tokenizer {
	...
	private HashMap<String, String> equivalents; // 11-July-2012
	...
	public ArrayList<TokenList> tokenize(MIPSprogram p) throws ProcessingException {
		sourceMIPSprogram = p;
		equivalents = new HashMap<String, String>(); // 11-July-2012
		ArrayList<TokenList> tokenList = new ArrayList<TokenList>();
		//ArrayList source = p.getSourceList();
		ArrayList<SourceLine> source = processIncludes(p, new HashMap<String, String>()); // 9-Jan-2013
		...
		for (int i = 0; i < source.size(); i++) {
			...
			// 03-Jan-2013. Related to 11-July-2012. If source code substitution was made
	...
	// 11-Jan-2013
	private ArrayList<SourceLine> processIncludes(MIPSprogram program, Map<String, String> inclFiles)
	...
	*/
	...
	public TokenList tokenizeLine(MIPSprogram program, int lineNum, String theLine, boolean doEqvSubstitutes) {
			...
			} else // not inside a quoted string, so be sensitive to delimiters
			{
				switch (c) {
					...
					case '-':
						...
						// (e.g. 1.2e-5) Add the + or - to the token and keep going.  17 Aug 2005
						...
						token[tokenPos++] = c;
						if (!((result.isEmpty()
								|| ((Token) result.get(result.size() - 1)).getType() != TokenTypes.IDENTIFIER) &&
		...
		if (doEqvSubstitutes) {
			result = processEqv(program, lineNum, theLine, result); // 11-July-2012
		}
		return result;
	}
	...
	private TokenList processEqv(MIPSprogram program, int lineNum, String theLine, TokenList tokens) {
		...
		tokens.setProcessedLine(theLine); // 03-Jan-2013. Related to changes of 11-July-2012.
