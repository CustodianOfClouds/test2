...
public class MarsLaunch {
	...
	private void dumpSegments() {
		...
		for (int i = 0; i < dumpTriples.size(); i++) {
			...
			// If not segment name, see if it is address range instead.  14-July-2008
			if (segInfo == null) {
				try {
	...
	private boolean runCommand() {
		...
		try {
			...
			ArrayList<MIPSprogram> MIPSprogramsToAssemble = code.prepareFilesForAssembly(filesToAssemble,
					mainFile.getAbsolutePath(),
					null);
			if (Globals.debug)
				out.println("--------  ASSEMBLY BEGINS  -----------");
			// Added logic to check for warnings and print if any. 11/28/06
			...
			if (startAtMain && SymbolTable.getStartLabelAddr() == SymbolTable.NOT_FOUND)
				...
				RegisterFile.initializeProgramCounter(startAtMain); // 3/9/09
				if (simulate) {
	...
	private void displayMemoryPostMortem() {
		...
		while (memIter.hasNext()) {
			...
			for (int addr = addressStart; addr <= addressEnd; addr += Memory.WORD_LENGTH_BYTES) {
				...
				try {
					// Allow display of binary text segment (machine code) 14-July-2008
