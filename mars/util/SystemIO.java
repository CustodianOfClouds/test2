...
public class SystemIO {
	...
	// Added by 28 Feb 2008.  See getInputReader() below.
	...
	public static String readString(int serviceNumber, int maxLength) {
		...
		if (input.length() > maxLength) {
			// Modified 13-July-2011.  Originally: return input.substring(0, maxLength);
	...
	*/
	public static int readFromFile(int fd, byte[] myBuffer, int lengthRequested) {
		...
		try {
			...
			// value represents an error, so we return 0 for EOF.  10-July-2008.
			if (retValue == -1)
				retValue = 0;
		} catch (IOException e) {
			fileErrorString = new String(
	...
	*/
	...
	// transparent to it.  Lazy instantiation. 28 Feb 2008
