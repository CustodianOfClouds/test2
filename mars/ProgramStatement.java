...
// my IDE gets pissy when adding type so whatever no type the code works ig
...
public class ProgramStatement {
		...
		for (int i = 1; i < strippedTokenList.size(); i++) {
			...
			if (tokenType == TokenTypes.REGISTER_NUMBER) {
				...
				if (address == SymbolTable.NOT_FOUND) // symbol used without being defined
				{
					...
					errors.add(
							new ErrorMessage(this.sourceMIPSprogram, token.getSourceLine(), token.getStartPos(), msg));
					return;
				}
				...
				//
				// 14 June 2007: Apply delayed branching if enabled.  This adds 4 bytes to the
				// address used to calculate branch distance in relative words.
				//
				// 4 January 2008: Apply the delayed branching 4-byte (instruction length) addition
	...
	public String getSourceFile() {
		return (originalProgram != null) ? originalProgram.getFilename()
	 ...
	 *   intended to be used when source code is available.  11-July-2013
	 */
